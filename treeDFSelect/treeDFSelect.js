...
Tree.prototype.DFSelect = function(filter) {
  const matches = [];
  let depth = 0;
  // first goal - read all numbers in order
  //then see if can filter through
  const filterDepths = (node) => {

    if (this.value) {
      matches.push ( filter(this.value))
    }

    if (this.children) {
      this.children.forEach(child => {
        depth++;
        matches.concat ( child.DFselect(filter, depth) );
      })
    }
  }
